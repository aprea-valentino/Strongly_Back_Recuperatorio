...
public CartResponse updateItemQty(Long userId, Long productId, int qty) {
    ...
    @Override
    @Transactional
    public CheckoutResponse checkout(Long userId) throws ProductNotFoundException {
        Cart cart = getCartByUser(userId);
    ...
    Product product = ProductRepository.findById(i.getProduct().getId()).orElseThrow(() -> new ProductNotFoundException("Producto no encontrado: " + i.getProduct().getId()));
